...
const transactionOption = {from:process.env.MANUFACTURER_ADDRESS, gas: process.env.DEFAULT_GAS}

var manufacturerProvider = ProviderManager.getHttpProvider(process.env.NETWORK, process.env.MANUFACTURER_PK);
...
Promise.all(contractsAsyncGets).then( async contracts => {
    ...
    async function assignSortingTask(){
        await ProductContract.authorizeMachine(process.env.SLD_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await SLDContract.assignSortingTask(1, process.env.DUMMY_PRODUCT, transactionOption);
        Logger.info("Sorting task assigned " + receipt.tx);
        finish()
    }

    async function assignProcessingTask(){
        await ProductContract.authorizeMachine(process.env.MPO_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await MPOContract.assignProcessingTask(1, process.env.DUMMY_PRODUCT, transactionOption);
    ...
    async function assignFetchContainerTask(){
        receipt = await HBWContract.assignFetchContainerTask(1, transactionOption);
        Logger.info("FetchContainer task assigned " + receipt.tx);
        finish()
    }

    async function assignStoreContainerTask(){
        receipt = await HBWContract.assignStoreContainerTask(1, transactionOption);
        Logger.info("FetchContainer task assigned " + receipt.tx);
        finish()
    }

    async function assignStoreProductTask(){
        await ProductContract.authorizeMachine(process.env.HBW_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await HBWContract.assignStoreProductTask(1, process.env.DUMMY_PRODUCT, "1", "11", transactionOption);
        Logger.info("FetchContainer task assigned " + receipt.tx);
        finish()
    }
    ...
    async function assignFetchProductTask(){
        await ProductContract.authorizeMachine(process.env.HBW_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await HBWContract.assignFetchProductTask(1, process.env.DUMMY_PRODUCT, transactionOption);
        Logger.info("FetchContainer task assigned " + receipt.tx);
        finish()
    }

    async function assignedGetInfoTask(){
        await ProductContract.authorizeMachine(process.env.VGR_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await VGRContract.assignGetInfoTask(1, process.env.DUMMY_PRODUCT, transactionOption);
    ...
    async function assignedDropToHBWTask(){
        await ProductContract.authorizeMachine(process.env.VGR_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await VGRContract.assignDropToHBWTask(1, process.env.DUMMY_PRODUCT, transactionOption);
    ...
    async function assignedMoveHBW2MPOTask(){
        await ProductContract.authorizeMachine(process.env.VGR_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await VGRContract.assignMoveHBW2MPOTask(1, process.env.DUMMY_PRODUCT, transactionOption);
    ...
    async function assignedPickSortedTask(){
        await ProductContract.authorizeMachine(process.env.VGR_ADDRESS, process.env.DUMMY_PRODUCT, {from:process.env.PRODUCT_OWNER_ADDRESS});
        receipt = await VGRContract.assignPickSortedTask(1, process.env.DUMMY_PRODUCT, "PINK", transactionOption);
