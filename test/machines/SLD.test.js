...
describe("SLD", function () {
    const [Admin, SLDOwner, MachineID, product, anyone, Manufacturer ] = accounts;

    beforeEach(async function () {
        this.ProductContract = await ProductArtifact.new({from: Admin});

        this.SLDContract = await SLDArtifact.new(SLDOwner, MachineID, this.ProductContract.address, {from: SLDOwner});
