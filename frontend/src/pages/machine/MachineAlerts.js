...
class MachineAlerts extends React.Component {
    ...
    getMachineAlerts(machine){
        ...
        MachineContract.methods["getAlertsCount"]().call().then( alertsCount => {
            for (let alertID = 1; alertID <= alertsCount; alertID++) {
                MachineContract.methods["getAlert"](alertID).call().then( alertResult => {
                                        ...
                                        :<Table>
                                            <Table.Header>
                                                <Table.Row>
                                                    ...
                                                    <Table.ColHeader alignContent="center">Reason</Table.ColHeader>
                                                    <Table.ColHeader alignContent="center">Time</Table.ColHeader>
                                                    <Table.ColHeader alignContent="center">Reading ID</Table.ColHeader>
                                                </Table.Row>
                                            </Table.Header>
                                            <Table.Body>
                                            {
                                                this.state.alerts.map((alert, i) =>
                                                    <Table.Row key={alert.ID}>
                                                        <Table.Col alignContent="center">{alert.ID}</Table.Col>
                                                        <Table.Col alignContent="center">{alert.type}</Table.Col>
                                                        <Table.Col alignContent="center">{alert.reason}</Table.Col>
                                                        <Table.Col alignContent="center">{alert.time}</Table.Col>
                                                        <Table.Col alignContent="center">{alert.readingID}</Table.Col>
                                                    </Table.Row>
                                                )
                                            }
                                            </Table.Body>
